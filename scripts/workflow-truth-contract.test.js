...
test("wave-prioritize bundles same-owned-root candidates into one lane", () => {
  const workflow = read("scripts/workflows/wave-prioritize.workflow.js");
  const partitionLanes = new Function(
    "ownedRoot",
    `${extractFunction(workflow, "partitionLanes")}\nreturn partitionLanes;`,
  )((row) => (row.issue.startsWith("org/a#") ? "curaos/backend/services/audit-core-service" : "curaos/backend/services/calendar-core-service"));

  const ranked = [
    { issue: "org/a#1", score: 0.9 },
    { issue: "org/a#2", score: 0.7 },
    { issue: "org/b#3", score: 0.6 },
  ];
  const lanes = partitionLanes(ranked, new Map(), Infinity);

  assert.deepEqual(lanes.map((lane) => lane.issue), ["org/a#1", "org/b#3"]);
  assert.deepEqual(lanes[0].issues, ["org/a#1", "org/a#2"]);
  assert.equal(lanes[0].bundled_count, 2);
  assert.deepEqual(lanes[1].issues, ["org/b#3"]);
});
...
test("submodule pointer-drift parser + init helper classify two-level status (#706 P5c)", () => {
  ...
  const okInit = wg.initSubmodulesRecursive({ gitFn: (args) => {
    ...
    if (args.includes("submodule") && args.includes("update")) return { ok: true, stdout: "" };
    if (args[0] === "submodule" && args[1] === "status") return { ok: true, stdout: " 6666666666666666666666666666666666666666 pkg/a (v1)" };
    return { ok: false, stdout: "", stderr: "unexpected" };
  } });
  assert.deepEqual(calls, ["-c protocol.file.allow=always submodule update --init --recursive --jobs 8", "submodule status --recursive"]);
  ...
  assert.deepEqual(customCalls, ["-c protocol.file.allow=always submodule update --init --recursive --jobs 3", "submodule status --recursive"]);

  // A failed init surfaces init_error but never throws.
  const badInit = wg.initSubmodulesRecursive({ gitFn: (args) => (args.includes("update") ? { ok: false, stderr: "network down" } : { ok: true, stdout: "" }) });
...
test("wave prioritize writes the wave-plan.json artifact with the three planning fields (RP-49)", (t) => {
  ...
  const { buildWavePlan } = new Function(
    `${extractFunction(prioritize, "issuesForLane")}\n${extractFunction(prioritize, "buildWavePlan")}\nreturn { buildWavePlan };`,
  )();
  const ranked = [
    { issue: "org/a#1", score: 0.9, unblockReach: 4, criticalPathDepth: 2 },
    { issue: "org/b#2", score: 0.7, unblockReach: 1, criticalPathDepth: 3 },
    { issue: "org/a#4", score: 0.6, unblockReach: 1, criticalPathDepth: 1 },
    { issue: "org/c#3", score: 0.5, unblockReach: 0, criticalPathDepth: 0 },
  ];
  const lanes = [
    { issue: "org/a#1", issues: ["org/a#1", "org/a#4"], bundled_count: 2, score: 0.9, owned_root: "curaos/backend/services/a" },
    { issue: "org/b#2", issues: ["org/b#2"], bundled_count: 1, score: 0.7, owned_root: "curaos/backend/services/b" },
    { issue: "org/c#3", issues: ["org/c#3"], bundled_count: 1, score: 0.5, owned_root: "workspace" },
  ];
  ...
  assert.deepEqual(plan.critical_path.map((r) => r.issue), ["org/b#2", "org/a#1", "org/a#4"]);
  // Field 3: velocity-sized scope from the calibration sizing signal.
  assert.deepEqual(plan.velocity_sized_scope, { suggestedWaveSize: 2, source: "calibration", scope: ["org/a#1", "org/a#4"] });
  ...
  assert.deepEqual(fallbackPlan.velocity_sized_scope, { suggestedWaveSize: null, source: "fallback-all-lanes", scope: ["org/a#1", "org/a#4", "org/b#2", "org/c#3"] });
...
assert.doesNotMatch(wave, /scan\.candidates\.length === 0 && Array\.isArray\(deterministicActiveScan\.candidates\)/);
  ...
  assert.match(wave, /function bundleIssueBodies\(issues, prefetchedMap\)/);
  assert.match(wave, /issueBodies: bundleIssueBodies\(issues, prefetched\)/);
  assert.match(wave, /\.\.\.\(combinedIssueBody \? \{ issue_body: combinedIssueBody \} : \{\}\)/);
...
test("dispatch schedules near-completion lanes before fresh lanes of equal priority (RP-51)", () => {
  ...
  const bundledLanes = [
    { issue: "o/fresh#4", issues: ["o/fresh#4"] },
    { issue: "o/bundle#5", issues: ["o/bundle#5", "o/resume#6"] },
  ];
  const bundledRanked = [
    { issue: "o/fresh#4", criticalPathDepth: 0, priority: "High" },
    { issue: "o/bundle#5", criticalPathDepth: 0, priority: "High" },
    { issue: "o/resume#6", criticalPathDepth: 0, priority: "High" },
  ];
  const bundledOrdered = orderLanes(bundledLanes, { ranked: bundledRanked, nearCompletion: ["o/resume#6"] });
  assert.deepEqual(bundledOrdered.map((l) => l.issue), ["o/bundle#5", "o/fresh#4"]);

  // Playbook documents the policy.
  assert.match(read("docs/agents/workflows/wave-prioritize.md"), /Lane scheduling \(RP-51/);
});

test("bundled wave lanes keep issue accounting and every acceptance criterion visible", () => {
  const wave = read("scripts/workflows/milestone-wave.workflow.js");
  const task = read("scripts/workflows/task-execute.workflow.js");
  assert.match(wave, /issues: output\.issues \|\| bundleIssues/);
  assert.match(wave, /issues: bundleIssues, status: "errored"/);
  assert.doesNotMatch(wave, /dispatched\.push\(\{ issue, status: "blocked"/);
  assert.doesNotMatch(wave, /return \{ issue, status: "(blocked|errored|pr-open)"/);
  const noOpBarrier = new Function(`${extractFunction(wave, "blockedByNoOpBarrier")}\nreturn blockedByNoOpBarrier;`)();
  assert.deepEqual(noOpBarrier("org/app#3", "org/app#1", ["org/app#3", "org/app#4"]).issues, ["org/app#3", "org/app#4"]);

  const taskHelpers = new Function(
    `${extractFunction(task, "valuesForSpec")}\n${extractFunction(task, "acceptanceForIssue")}\n${extractFunction(task, "combineIssueSpec")}\nreturn { combineIssueSpec };`,
  )();
  const waveHelpers = new Function(
    `${extractFunction(wave, "valuesForIssueSpec")}\n${extractFunction(wave, "acceptanceForIssue")}\n${extractFunction(wave, "combineLaneIssueSpec")}\nreturn { combineLaneIssueSpec };`,
  )();
  const rows = [
    { issue: "org/app#1", ctx: { issue_spec: { owned_paths: ["a"], acceptance: ["ship A"] } } },
    { issue: "org/app#2", ctx: { issue_spec: { owned_paths: ["a"], acceptance: ["ship B"] } } },
  ];
  for (const helper of [taskHelpers.combineIssueSpec, waveHelpers.combineLaneIssueSpec]) {
    const spec = helper(rows);
    assert.deepEqual(spec.acceptance, ["org/app#1: ship A", "org/app#2: ship B"]);
    assert.deepEqual(spec.bundled_issues, ["org/app#1", "org/app#2"]);
  }
});
