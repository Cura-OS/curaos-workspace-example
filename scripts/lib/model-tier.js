...
// Sonnet ONLY when the task is proven-simple; Haiku ONLY for pure-mechanical. Keep Opus as
// the frontier tier while Fable is unavailable, reserved for architecture-defining work:
// XL effort AND ADR-involved (a wrong structural call cascades).
//
 ...
 * Escalate to OPUS only when architecture-defining: effort === "XL" AND adr_refs.length > 0
 *   (frontier reasoning justified, so the gate is deliberately narrow).
...
function pickImplementModel(issueSpec, scopeHint) {
  ...
  // Architecture-defining -> opus (frontier tier). BOTH must hold: XL effort AND ADR involvement.
