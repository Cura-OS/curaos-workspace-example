...
function queryRows(dbPath, sql) {
  fs.mkdirSync(path.dirname(dbPath), { recursive: true });
  const out = execFileSync("sqlite3", ["-cmd", ".timeout 5000", "-json", dbPath, sql], { encoding: "utf8" }).trim();
...
function listIssues({
  ...
  if (parsedLimit !== null && (!Number.isInteger(parsedLimit) || parsedLimit < 1)) {
    throw new Error(`invalid list limit: ${limit}`);
  }
  const where = clauses.length ? `WHERE ${clauses.join(" AND ")}` : "";
  const suffix = parsedLimit === null ? "" : ` LIMIT ${parsedLimit}`;
  return queryRows(dbPath, `SELECT * FROM local_issues ${where} ORDER BY created_at, id${suffix}`);
}
