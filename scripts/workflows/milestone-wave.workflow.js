...
function blockedByNoOpBarrier(issue, failedIssue, issues = []) {
  return {
    issue,
    issues: issues.length ? issues : [issue],
    status: "blocked",
    pr: "",
...
function laneLead(lane) {
  if (lane && typeof lane === "object") return String(lane.issue || "");
  return String(lane || "");
}

function laneIssueList(lane) {
  ...
  const issue = laneLead(lane);
  return issue ? [issue] : [];
}

function bundleIssueBodies(issues, prefetchedMap) {
  const bodies = {};
  for (const issue of issues) {
    const record = prefetchedMap && prefetchedMap.has(issue) ? prefetchedMap.get(issue) : null;
    if (record && record.body) bodies[issue] = record.body;
  }
  return bodies;
}

function valuesForIssueSpec(rows, key) {
  const out = [];
...
function combineLaneIssueSpec(rows) {
  const primary = rows[0] && rows[0].ctx && rows[0].ctx.issue_spec ? rows[0].ctx.issue_spec : {};
  const acceptance = rows.length === 1
    ? valuesForIssueSpec(rows, "acceptance")
    : rows.flatMap((row) => acceptanceForIssue(row.issue, row.ctx && row.ctx.issue_spec ? row.ctx.issue_spec.acceptance : []));
  return {
    ...primary,
    acceptance,
    owned_paths: valuesForIssueSpec(rows, "owned_paths"),
    closeout_paths: valuesForIssueSpec(rows, "closeout_paths"),
    forbidden_paths: valuesForIssueSpec(rows, "forbidden_paths"),
    verification_cmds: valuesForIssueSpec(rows, "verification_cmds"),
    adr_refs: valuesForIssueSpec(rows, "adr_refs"),
...
function combineLaneContextSummary(rows) {
...
function combineLaneIssueBodies(issues, bodies) {
  const blocks = [];
  for (const issue of issues) {
    if (bodies && bodies[issue]) blocks.push(`## ${issue}\n${String(bodies[issue]).trim()}`);
  }
  return blocks.join("\n\n");
}
...
function initLaneSubmodules(worktreePath, jobs) {
  ...
  execFileSync("git", ["-C", worktreePath, "-c", "protocol.file.allow=always", "submodule", "update", "--init", "--recursive", "--jobs", String(jobCount)], {
...
function runTaskExecuteChild({ issue, issues, branch, worktreePath, agentType, timeoutMs, issueBodies }) {
  const { spawn } = process.getBuiltinModule("node:child_process");
  const bundleIssues = Array.isArray(issues) && issues.length ? issues : [issue];
  const argsJson = JSON.stringify({
    issue,
    ...(bundleIssues.length > 1 ? { bundle_issues: JSON.stringify(bundleIssues) } : {}),
    ...(issueBodies && Object.keys(issueBodies).length ? { bundle_issue_bodies: JSON.stringify(issueBodies) } : {}),
    branch,
    branch_precreated: true,
    dry_run: false,
  });
  const argv = [
    "workflow-run",
  ...
  return new Promise((resolve) => {
    ...
    child.on("error", (error) => {
      resolve({ issue, issues: bundleIssues, status: "errored", pr: "", branch, worktree: worktreePath, blocker: `task-execute child spawn failed: ${error.message}` });
    });
    child.on("close", (code) => {
      if (code !== 0) {
        ...
        resolve({ issue, issues: bundleIssues, status: "errored", pr: "", branch, worktree: worktreePath, blocker: `task-execute child exit ${code}${evidence ? `: ${evidence}` : ""}`, child_run_id: childRunId });
        return;
      }
      try {
        const parsed = parseWorkflowJson(stdout);
        ...
        resolve({
          issue,
          issues: output.issues || bundleIssues,
        ...
        resolve({ issue, issues: bundleIssues, status: "errored", pr: "", branch, worktree: worktreePath, blocker: `task-execute child json parse failed: ${error.message}` });
      }
    });
  });
}
...
if (readyLeaves.length) {
  ...
  // wave-prioritize already partitioned parallel-safe lanes in leverage order. A lane may now
  // bundle multiple same-owner issues; keep the lane object so the worker sees the whole slice.
  const laneObjects = (prio.lanes || []).filter((lane) => laneLead(lane));
  const includedIssues = new Set(laneObjects.flatMap((lane) => laneIssueList(lane)));
  log(`Prioritized ${readyLeaves.length}: ${prio.rationale || "(no rationale)"} | dispatching ${laneObjects.length} bundle lane(s) covering ${includedIssues.size} issue(s) in leverage order`);
  partition = { lanes: laneObjects, held: [...generatorBarrier.held, ...(prio.ranked || []).filter((r) => !includedIssues.has(r.issue)).map((r) => ({ issue: r.issue, reason: "deferred: lane collision or over max_lanes (lower leverage)" }))] };
}
...
if (!dryRun && partition.lanes.length) {
  try {
    ...
    if (decision.action === "defer") {
      partition.held.push(...partition.lanes.flatMap((lane) => laneIssueList(lane).map((issue) => ({ issue, reason: `budget-preflight deferral: ${decision.reason}` }))));
      partition.lanes = [];
    }
  } catch (error) {
...
if (!dryRun && partition.lanes.length) {
  ...
  if (dispatchMode !== "serial") {
    ...
    for (const lane of partition.lanes) {
      const issue = laneLead(lane);
      const issues = laneIssueList(lane);
      const laneText = issues.map((ref) => {
        const laneCtx = readyCandidateContext.get(ref) || {};
        return [ref, laneCtx.module, laneCtx.owned_path].filter(Boolean).join(" ");
      }).join(" ");
      if (isDeployCredentialScope(issue, laneText)) {
        const pf = preflightCredentialsGate();
        if (pf.exit !== 0) {
          log(`DEPLOY-PREFLIGHT: holding ${issue} (preflight-credentials exit ${pf.exit})`);
          dispatched.push({ issue, issues, status: "blocked", pr: "", blocker: `deploy-credential-preflight failed (exit ${pf.exit}: 65=missing scope, 66=registry write rejected, 70=cannot prove): operator unblock = gh auth refresh -h github.com -s write:packages,read:packages && scripts/preflight-credentials${pf.output ? `; ${pf.output.slice(0, 400)}` : ""}` });
          continue;
        }
      }
      ...
      if (!wt.branch || wt.blocker) {
        dispatched.push({ issue, issues, status: "blocked", pr: "", branch: wt.branch || branchName, worktree: wt.path || "", blocker: `worktree-create-failed: ${wt.blocker || "unknown"}` });
        continue;
      }
      ...
      try {
        ...
        dispatched.push({ issue, issues, status: "blocked", pr: "", branch: branchName, worktree: worktreePath, blocker: `worktree-submodule-init-failed: ${error && error.message ? error.message : String(error)}` });
        continue;
      }
      log(`DISPATCH-WORKTREE-LANE: queued ${issue} branch=${branchName} worktree=${worktreePath}`);
      lanePlans.push({ issue, issues, issueBodies: bundleIssueBodies(issues, prefetched), branch: branchName, worktreePath });
    }
  ...
  const runLane = async (lane) => {
    ...
    try {
      ...
      const contextRows = [];
      for (const bundledIssue of issues) {
        const ctx = await workflow({ scriptPath: `${WF}/context-load.workflow.js` }, { issue: bundledIssue, scope_hint: "" });
        if (ctx.blockers && ctx.blockers.length) return { issue, issues, status: "blocked", pr: "", blocker: `${bundledIssue}: ${ctx.blockers.join("; ")}` };
        contextRows.push({ issue: bundledIssue, ctx });
      }
      const primaryCtx = contextRows[0].ctx;
      const bundleBodies = bundleIssueBodies(issues, prefetched);
      const combinedIssueBody = combineLaneIssueBodies(issues, bundleBodies);
      ...
      if (branch.branch !== branchName) {
        ...
        return { issue, issues, status: "blocked", pr: "", blocker: `branch-create-failed: expected ${branchName}, ${branch.blocker || "git branch creation failed"}${restoreSuffix(restore)}` };
      }
      ...
      const impl = await workflow({ scriptPath: `${WF}/tdd-implement.workflow.js` }, {
        issue,
        branch: branchName,
        context_summary: `${anchorBlock}\n\n${combineLaneContextSummary(contextRows)}`,
        generated_code: contextRows.some((row) => row.ctx.generated_code === true),
        issue_spec: combineLaneIssueSpec(contextRows),
      ...
      if (impl.status !== "done") {
        ...
        const workflowDefect = impl.workflow_defect === true
          ...
          : "";
        return { issue, issues, status: impl.status === "needs-user" ? "needs-user" : "blocked", pr: "", workflow_defect: impl.workflow_defect === true, workflow_defect_kind: impl.workflow_defect_kind || "", blocker: `${workflowDefect}${impl.blocker || "tdd-implement did not reach done"}${restoreSuffix(restore)}` };
      }
      if (!impl.verification_evidence || impl.verification_evidence.trim().length < 40) {
        ...
        return { issue, issues, status: "blocked", pr: "", workflow_defect: impl.workflow_defect === true, workflow_defect_kind: impl.workflow_defect_kind || "", blocker: `tdd-implement reached done without §8.1 verification_evidence${restoreSuffix(restore)}` };
      }
      const pr = await agent(
        `From ${ROOT} on branch ${branchName}: push the branch and open one PR for bundled lane issues ${issues.join(", ")} with \`env -u GITHUB_TOKEN gh pr create\`. The PR body MUST be generated from the ACTUAL change - run \`git diff --stat ${resolveDefaultBranch()}...${branchName}\` and describe ONLY what the diff shows; do NOT claim work absent from the diff. Include: link every bundled issue; the real changed-file summary; the verification_evidence block below (verbatim, the §8.1 claim of record; it may include an INDEPENDENT VERIFICATION fallback paste); "GENERATOR-EVOLUTION: ${impl.generator_evolution}". If VERIFICATION_EVIDENCE is missing or says "(none provided)", STOP and report no PR. Before pushing, if the diff moves any submodule pointer, verify the pointed commit is pushed/reachable (\`git -C <submodule> branch -r --contains HEAD\` non-empty) - if not, do NOT push; report status reflecting the broken pointer. Then set every bundled issue label to agent-PR-open (remove agent-claimed:*). Conventional Commit title; NO AI attribution trailers. Report the PR ref (owner/repo#N).\n\nVERIFICATION_EVIDENCE (paste verbatim into the PR body):\n${impl.verification_evidence}`,
      ...
      if (!prRef) {
        return { issue, issues, status: "blocked", pr: "", blocker: `pr-create-failed: agent returned <empty-or-invalid>${restoreSuffix(restore)}` };
      }
      if (restore.restored !== true) {
        return { issue, issues, status: "blocked", pr: prRef, workflow_defect: impl.workflow_defect === true, workflow_defect_kind: impl.workflow_defect_kind || "", blocker: `post-pr default-branch restore failed${restoreSuffix(restore)}` };
      }
      return { issue, issues, status: "pr-open", pr: prRef, blocker: "" };
    } catch (e) {
      return { issue, issues, status: "errored", pr: "", blocker: String(e) };
    }
  };
  for (let index = 0; index < partition.lanes.length; index += 1) {
    const lane = partition.lanes[index];
    const issue = laneLead(lane);
    const issues = laneIssueList(lane);
    ...
    if (isNoOpWorkflowDefect(result)) {
      ...
      for (const heldLane of remaining) {
        const heldIssue = laneLead(heldLane);
        dispatched.push(blockedByNoOpBarrier(heldIssue, issue, laneIssueList(heldLane)));
      }
      break;
    }
  }
  ...
  dispatched = partition.lanes.map((lane) => ({ issue: laneLead(lane), issues: laneIssueList(lane), status: "dry-run (would dispatch)", pr: "", blocker: "" }));
}
