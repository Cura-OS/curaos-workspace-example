...
description: MCP stack curation - CLI-first principle (use Bash + installed CLI over MCP wherever CLI exists); must-have MCPs (open-design + context-mode; deepwiki already-enabled nice-to-have; computer-use Codex bundled); per-session activation pattern; tool-search subagent (Claude Code 2.0.6+) when >3 active MCPs; explicit ban list (Grafana/GitHub/Playwright/Kubernetes/Semgrep/Serena/Figma/Linear/Jira/MongoDB/Datadog/Cloudflare-*/git-MCP/Filesystem-MCP/Fetch-MCP/all memory MCPs); HARD BAN Postgres MCP→prod PHI + Playwright MCP→HealthStack; GitHub Issues as canonical work queue
metadata:
  node_type: memory
  type: feedback
...
2. **Must-have MCPs (always-on)** - open-design + context-mode (Codex-bundled computer-use kept where applicable); deepwiki kept as already-enabled cheap addon. CodeGraph was retired on 2026-09-21 in favour of the search ladder: zvec-grep when available; the language server and ast-grep always, combined with zvec-grep; normal search tools only when none of those is available
...
| Must-have MCP filter | Only open-design (local app canvas) + context-mode (huge-output protection) + deepwiki (OSS wiki Q&A) have NO usable CLI substitute |
...
| **tree-sitter MCP** | language server + ast-grep (`sg` CLI) cover structural queries |
| **Sourcegraph MCP** | zvec-grep managed `rg` + ast-grep for cross-repo |
...
| **Sourcegraph MCP** | Enterprise license + cloud dep; language server + ast-grep + zvec-grep cover local-repo case |
...
- open-design + context-mode + deepwiki (Claude Code)
- open-design + context-mode + computer-use (Codex)
...
| Rule | Compliance |
|---|---|
| AGENTS.md §3 charter (self-hosted first) | All must-have MCPs are local (open-design local app, context-mode plugin); deepwiki cloud but cheap |
...
- `~/.claude/settings.json`: must-have MCPs only (open-design + context-mode + deepwiki)
- `~/.codex/config.toml`: must-have MCPs only (open-design + context-mode + computer-use)
