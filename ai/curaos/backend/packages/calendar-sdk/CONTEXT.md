# `@curaos/calendar-sdk` — agent context

## Why this package exists (M10 #283, Epic #24)

Any consumer service needs a typed client for `calendar-core-service` without hand-writing
...
```
calendar-core-service/specs/calendar.tsp
  --(tsp compile, @typespec/openapi3)-->  calendar-core-service OpenAPI 3.1
  --(@hey-api/openapi-ts + @hey-api/client-fetch)-->  src/rest/  (REST client)

calendar-core-service/specs/calendar.asyncapi.yaml
...
| Export | Purpose |
|---|---|
| `calendarsHealth`, `calendarsProtectedProbe`, `calendarsProtectedWrite`, `calendarsRead` | Typed REST operation functions |
...
- **Consumes (build-time)** — `calendar-core-service/specs/calendar.tsp` + `calendar-core-service/specs/calendar.asyncapi.yaml`
  (contracts) → generated into `src/`.
- **Consumed by** — downstream services + the M10 capstone integration test
  (#285); any consumer that calls calendar-core-service or consumes its events.
- **Must-not-break files** — `calendar-core-service/specs/*` (the contracts). A change there
...
- **Gates with package-specific handling** —
  ...
  - `.dependency-cruiser.cjs` carve-out for `backend/packages/calendar-sdk/src/rest/(client|core)/*.gen.ts`
