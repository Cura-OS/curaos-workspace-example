# `@curaos/settings-sdk` — agent context

## Why this package exists (M10 #281, Epic #24)

Any consumer service needs a typed client for `settings-service` without hand-writing
...
```
settings-service/specs/settings.tsp
  --(tsp compile, @typespec/openapi3)-->  settings-service OpenAPI 3.1
  --(@hey-api/openapi-ts + @hey-api/client-fetch)-->  src/rest/  (REST client)

settings-service/specs/settings.asyncapi.yaml
...
| Export | Purpose |
|---|---|
| `settingssHealth`, `settingssProtectedProbe`, `settingssProtectedWrite`, `settingssRead` | Typed REST operation functions |
...
- **Consumes (build-time)** — `settings-service/specs/settings.tsp` + `settings-service/specs/settings.asyncapi.yaml`
  (contracts) → generated into `src/`.
- **Consumed by** — downstream services + the M10 capstone integration test
  (#285); any consumer that calls settings-service or consumes its events.
- **Must-not-break files** — `settings-service/specs/*` (the contracts). A change there
...
- **Gates with package-specific handling** —
  ...
  - `.dependency-cruiser.cjs` carve-out for `backend/packages/settings-sdk/src/rest/(client|core)/*.gen.ts`
