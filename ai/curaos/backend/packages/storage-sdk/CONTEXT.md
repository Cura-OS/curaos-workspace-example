# `@curaos/storage-sdk` — agent context

## Why this package exists (M10 #279, Epic #24)

Any consumer service needs a typed client for `storage-service` without hand-writing
...
```
storage-service/specs/storage.tsp
  --(tsp compile, @typespec/openapi3)-->  storage-service OpenAPI 3.1
  --(@hey-api/openapi-ts + @hey-api/client-fetch)-->  src/rest/  (REST client)

storage-service/specs/storage.asyncapi.yaml
...
| Export | Purpose |
|---|---|
| `storagesHealth`, `storagesProtectedProbe`, `storagesProtectedWrite`, `storagesRead` | Typed REST operation functions |
...
- **Consumes (build-time)** — `storage-service/specs/storage.tsp` + `storage-service/specs/storage.asyncapi.yaml`
  (contracts) → generated into `src/`.
- **Consumed by** — downstream services + the M10 capstone integration test
  (#285); any consumer that calls storage-service or consumes its events.
- **Must-not-break files** — `storage-service/specs/*` (the contracts). A change there
...
- **Gates with package-specific handling** —
  ...
  - `.dependency-cruiser.cjs` carve-out for `backend/packages/storage-sdk/src/rest/(client|core)/*.gen.ts`
