# `@curaos/search-sdk` — agent context

## Why this package exists (M10 #280, Epic #24)

Any consumer service needs a typed client for `search-service` without hand-writing
...
```
search-service/specs/search.tsp
  --(tsp compile, @typespec/openapi3)-->  search-service OpenAPI 3.1
  --(@hey-api/openapi-ts + @hey-api/client-fetch)-->  src/rest/  (REST client)

search-service/specs/search.asyncapi.yaml
...
| Export | Purpose |
|---|---|
| `searchsHealth`, `searchsProtectedProbe`, `searchsProtectedWrite`, `searchsRead` | Typed REST operation functions |
...
- **Consumes (build-time)** — `search-service/specs/search.tsp` + `search-service/specs/search.asyncapi.yaml`
  (contracts) → generated into `src/`.
- **Consumed by** — downstream services + the M10 capstone integration test
  (#285); any consumer that calls search-service or consumes its events.
- **Must-not-break files** — `search-service/specs/*` (the contracts). A change there
...
- **Gates with package-specific handling** —
  ...
  - `.dependency-cruiser.cjs` carve-out for `backend/packages/search-sdk/src/rest/(client|core)/*.gen.ts`
