---
name: procurement-core-service
description: "Neutral procure-to-pay primitives - PR→PO→receipt→3-way match + budget ledger."
...
docs:
  ...
  context: ai/curaos/backend/services/procurement-core-service/CONTEXT.md
  requirements: ai/curaos/backend/services/procurement-core-service/Requirements.md
---

# procurement-core-service - Agent Contract

> Neutral procure-to-pay primitives - PR→PO→receipt→3-way match + budget ledger (#343). Domain overlay: `neutral`.
## Mission

Provide vertical-agnostic procurement primitives reused by `personal-procurement-service` + `business-procurement-service` and any future overlay. Owns the procure-to-pay spine (purchase requisition → purchase order → receipt → 3-way match (PO ↔ receipt ↔ invoice) + append-only budget ledger), the `curaos.core.procurement.*` event contracts (emitted through the durable DOMAIN outbox), and the canonical storage shape. 
...
overlay_consumers:
  - personal-procurement-service
  - business-procurement-service
notable:
