---
name: orders-service
description: "Neutral orders primitives (FHIR R4 ServiceRequest/Task CPOE state model + Observation/DiagnosticReport results) - healthstack baseline."
...
docs:
  ...
  context: ai/curaos/backend/services/orders-service/CONTEXT.md
  requirements: ai/curaos/backend/services/orders-service/Requirements.md
---

# orders-service - Agent Contract

> Neutral primitives for orders (FHIR R4 ServiceRequest/Task CPOE state model + Observation/DiagnosticReport results). Domain overlay: `healthstack`.
## Mission

Provide vertical-agnostic orders primitives and any future overlay. Owns canonical orders contracts, events, storage shape. - PHI boundary: never persist PHI outside this service's overlay schema (HIPAA).
