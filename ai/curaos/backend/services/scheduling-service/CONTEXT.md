...
write-surface DTO (`src/.../scheduling.dto.ts`) — use them, never hand-roll
`z.number().int()`:
...
- APIs: REST `/api/v1/scheduling` (TBD path conventions)
## Open Questions
...
- `ai/curaos/backend/services/scheduling-service/Requirements.md` — full spec
